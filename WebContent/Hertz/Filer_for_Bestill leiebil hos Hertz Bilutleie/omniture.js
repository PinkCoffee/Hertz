...
/* Transfer custom code from old file - start */
//window.onload = getHostNameFromURL();
function getHostNameFromURL() {
	var url = window.location.toString();
	if (url) {
		var arrURL = url.split('/');
		var hostName = arrURL[2];
		//alert(hostName);
		return hostName;
	}else {
...
s.server = getHostNameFromURL();
/* Transfer custom code from old file - end */
...
s.linkInternalFilters="javascript:,hertz.com,hertz"
...
//* Form Analysis Config */
	s.formList="selectLanguageTopNav,resOnHomepage,resForm,whatForm,prepaidRMCForm,theform,ignoreChangesForm,myForm,loginForm,cancelCreateModifyUserIDForm,personalPage,eConsentView,termsAndConditionsView,vehicleDataView,affiliationDeleteCdp,affiliationAddCdp,affiliationDeleteFtp,affiliationAddFtp,saveAffiliation,eSignatureView,loginFormName,createModifyUserIDForm,validateRedemption"
	...
	/* adesai: customized code to show page title */
	function showTitle()
	{
	...
	if(!s.pageType && !s.pageName){
        ...
        /* s.pageName=showTitle(); */
	    if(s.getQueryParam('targetPage')!=""){
	   		s.pageName=s.getQueryParam('targetPage');
		}
    }       
	
	// Set trackingPageName variable on non framework supported page to enable omniture page tracking
	if((s.pageName == null || s.pageName == "") && trackingPageName != null && trackingPageName != "") {
		s.pageName=trackingPageName;
	}
	
	s.prop8=s.getTimeParting('h','-5','2008'); // Set hour 
	s.prop9=s.getTimeParting('d','-5','2008'); // Set day
	s.prop10=s.getTimeParting('w','-5','2008'); // Set Weekend / Weekday
	s.eVar16=s.prop10 + " - " + s.prop9 + " - " + s.prop8;
...
s.visitorNamespace="hertz"
s.trackingServer="hertz.122.2o7.net"
...
function s_giqf(){var w=window,q=w.s_giq,i,t,s;if(q)for(i=0;i<q.length;i++){t=q[i];s=s_gi(t.oun);s.sa(t.un);s.setTagContainer(t.tagContainerName)}w.s_giq=0}s_giqf()
		...
		var WebTracking=function(){var e={};return{events:{},getHTZReferrer:function(b){for(var b=b+"=",a=document.cookie.split(";"),c=0;c<a.length;c++){for(var d=a[c];" "==d.charAt(0);)d=d.substring(1,d.length);if(0==d.indexOf(b))return d.substring(b.length,d.length)}return null},getActionPageName:function(b){var a=s.getPageName().split(":");return b?(b=""!=s.getQueryParam("learnMore")?s.getQueryParam("learnMore"):"all",a[a.length-1].split(".")[0]+"_"+b):0<a.length?a[a.length-1].split(".")[0]+"_"+a[a.length-
3]+"_"+a[a.length-2]:"ERROR_ACTION_NAME"},isDuplicateRequest:function(b){if(e[b])return!0;e[b]=!0;return!1},sendTrackingData:function(b,a,c){if(c||!this.isDuplicateRequest(b)){c=s_gi(s_account);c.eVar22="";for(var d in a)c[d]=a[d];c.tl(!0,"o",b)}},sendTrackingRequest:function(b,a){this.sendTrackingData(b,a,!1)},sendPageTrackingRequest:function(b){if(b){var a=s_gi(s_account);a.eVar22="";a.pageName=b;void a.t()}},setValue:function(b,a){s[b]=a},setError:function(b,a){this.trackErrors(b,{pageName:a?a:
htz.config.legacyPage})},setErrorInfo:function(){"undefined"!==typeof htz&&htz.pageError&&htz.pageError.hasError&&(s.pageName="error-"+s.getQueryParam("targetPage"),s.eVar22=htz.pageError.errorCodes,"undefined"!=typeof _gaq&&_gaq.push(["_trackEvent","ERROR",s.pageName,s.eVar22]))},trackErrors:function(b,a){if(b){a||(a={});try{a=$.extend(a,{eVar22:b,pageName:"error-"+a.pageName})}catch(c){}this.trackData(a);"undefined"!=typeof _gaq&&_gaq.push(["_trackEvent","ERROR",a.pageName,b])}},trackPage:function(b,
a){if(b){a||(a={});try{a=$.extend(a,{pageName:b})}catch(c){}this.trackData(a)}},trackData:function(b){if(b)try{var a=s_gi(s_account);a.eVar22="";for(var c in b)a[c]=b[c];void a.t()}catch(d){}}}}();                                                                  
